...
namespace BioMetrixCore
{
    internal class DeviceManipulator
    {
        ...
        public ICollection<MachineInfo> GetCheckInLogData(ZkemClient objZkeeper, int machineNumber)
        {
            ...
            try
            {
                ...
                //objZkeeper.ReadLastestLogData(machineNumber, 1, DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 0, 0, 0);
                ...
                while (objZkeeper.SSR_GetGeneralLogData(machineNumber, out dwEnrollNumber1, out dwVerifyMode, out dwInOutMode, out dwYear, out dwMonth, out dwDay, out dwHour, out dwMinute, out dwSecond, ref dwWorkCode))
                {
                    ...
                    if (dwDay >= DateTime.Now.Day && dwMonth == DateTime.Now.Month && dwYear == DateTime.Now.Year)
                    {
                        ...
                        objInfo.RegisterID = int.Parse(dwEnrollNumber1);
            ...
            catch (Exception exc)
            {
                ...
                LogWriter.WriteLog("Exception (591) : Details as follows" + exc.ToString());
                LogWriter.WriteLog("---------------------------------------------------------------------------");
        ...
        public ICollection<MachineInfo> GetCheckOutdata(ZkemClient objZkeeper, int machineNumber)
        {
            string dwEnrollNumber1 = "";
            int dwVerifyMode = 0;
            int dwInOutMode = 0;
            ...
            {
                //objZkeeper.ReadAllGLogData(machineNumber);
                //objZkeeper.ReadLastestLogData(machineNumber, 1000000, DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 17, 0, 0);
                while (objZkeeper.SSR_GetGeneralLogData(machineNumber, out dwEnrollNumber1, out dwVerifyMode, out dwInOutMode, out dwYear, out dwMonth, out dwDay, out dwHour, out dwMinute, out dwSecond, ref dwWorkCode))
                {
                    string inputDate = new DateTime(dwYear, dwMonth, dwDay, dwHour, dwMinute, dwSecond).ToString();
            ...
            {
                LogWriter.WriteLog("Exception Function Name : " + MethodBase.GetCurrentMethod().Name);
                LogWriter.WriteLog("Exception (59) : Details as follows" + exc.ToString());
                LogWriter.WriteLog("---------------------------------------------------------------------------");
                LogWriter.WriteLog("Exception : " + exc.ToString());
                LogWriter.WriteLog("Exception Stack trace : " + exc.StackTrace.ToString());
                LogWriter.WriteLog("Exception Message : " + exc.Message.ToString());
