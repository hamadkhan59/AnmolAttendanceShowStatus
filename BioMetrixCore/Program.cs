    ...
    static class Program
    {
        ...
        static void SendAttendanceData()
        {
            try
            {
                ...
                //{
                LogWriter.WriteLogStart("Connecting device at IP : PORT (" + ipAddress + ":" + portNumber + ")");
                ...
                if (isDeviceConnected)
                {
                    ...
                    //ICollection<MachineInfo>  lstMachineInfo_filtered = lstMachineInfo.ToList();
                    LogWriter.WriteLog("Fetched device logs count : " + (lstMachineInfo_filtered == null ? 0 : lstMachineInfo_filtered.Count));
                    if (lstMachineInfo_filtered != null && lstMachineInfo_filtered.Count > 0)
                    {
                        LogWriter.WriteLog("Fetched device logs current count : " + (lstMachineInfo_filtered == null ? 0 : lstMachineInfo_filtered.Count));
                        ...
                        foreach (MachineInfo info in lstMachineInfo_filtered1)
                        {
                            ...
                            if (info.TimeOnlyRecord != lastDateTime)
                            {
                                ...
                                int eventCount = lstMachineInfo.Where(x => x.RegisterID == info.RegisterID).Count();
                                ShowStatus(attendanceTime, "", info.RegisterID.ToString(), info.EnployeeName, eventCount);
        ...
        private static void ShowStatus(string attendanceTime, string result, string Id, string Name, int eventCount)
        {
            try
            {
                string message = "";
                ...
                if (message.Length > 0)
                {
                    ...
                    //form.Show();
                    System.Windows.Forms.Application.Run(new AttendanceForm(Id, Name, message, FormInterval, eventCount));
                    //Id = id;
                    //Name = name;
                    //Message = message;
                    //statusFlag = true;
